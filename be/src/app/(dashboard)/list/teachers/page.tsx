...
const columns = [
  ...
  { header: "Mã giáo viên", accessor: "teacherId", className: "hidden md:table-cell" },
  { header: "Môn dạy", accessor: "subjects", className: "hidden md:table-cell" },
  { header: "Lớp phụ trách", accessor: "classes", className: "hidden md:table-cell" },
  ...
  { header: "Ngày tạo", accessor: "createdAt", className: "hidden md:table-cell" },
...
const renderRow = (item: TeacherList) => (
  <tr key={item.id} className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurpleLight">
    ...
    <td className="hidden md:table-cell">{item.subjects.length > 0 ? item.subjects.map((s) => s.name).join(", ") : "-"}</td>
    ...
    <td className="hidden md:table-cell">{new Date(item.createdAt).toLocaleDateString("vi-VN")}</td>
    <td>
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">Danh sách giáo viên</h1>
