...
const SubjectListPage = async ({
  ...
  // Lấy dữ liệu từ DB
  const [data, count, teachersData] = await prisma.$transaction([
    ...
    prisma.teacher.findMany(), // lấy danh sách giáo viên để truyền vào FormModal
  ]);
    ...
    >
      ...
      <td className="hidden md:table-cell">
        {item.teachers.length > 0
          ? item.teachers.map((t) => `${t.surname} ${t.name}`).join(", ")
          : "-"}
      </td>
          ...
          {role === "admin" && (
            <>
              <FormModal
                ...
                type="update"
                data={item}
                relatedData={{ teachers: teachersData }}
              />
